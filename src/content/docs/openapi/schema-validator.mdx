...
## 👨‍💻 Example - HTTP Server POST Request
Here’s an example of how you can use the generated spec within an HTTP server POST request:
```go
func init() {
	ioc.Registry(
		postCustomer,
...
func postCustomer(e serverwrapper.EchoWrapper, spec openapi.CreateCustomerSpec) {
	e.POST("/insert-your-custom-pattern-here", func(c echo.Context) error {
		...
		if err := spec.ValidateBytes(bodyBytes); err != nil {
			return c.JSON(http.StatusBadRequest, map[string]string{
				"error": err.Error(),
			})
		}
